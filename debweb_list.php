...
/**
 *   	\file       debweb_list.php
 *		\ingroup    debweb
 *		\brief      List page for debweb
 */
...
require_once __DIR__.'/class/debweb.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('debweb', 'debweb', 'write');
	$permissiontodelete = $user->hasRight('debweb', 'debweb', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("debweb")) {
	accessforbidden('Module debweb not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'DebWeb';
	$objectlabel = 'DebWeb';
	$uploaddir = $conf->debweb->dir_output;
...
$title = $langs->trans("DebWebs");
//$help_url = "EN:Module_DebWeb|FR:Module_DebWeb_FR|ES:Módulo_DebWeb";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/debweb/debweb_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-debweb page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/debweb/debweb_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDebWebRef";
$modelmail = "debweb";
$objecttmp = new DebWeb($db);
