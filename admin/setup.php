...
/**
 * \file    debweb/admin/setup.php
 * \ingroup debweb
 * \brief   DebWeb setup page.
 */
...
require_once '../lib/debweb.lib.php';
...
//$item = $formSetup->newItem('DEBWEB_MYPARAM1');
...
//$item = $formSetup->newItem('DEBWEB_MYPARAM4');
//$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
//$formSetup->newItem('DEBWEB_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an email template of type thirdparty
//$formSetup->newItem('DEBWEB_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('DEBWEB_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a product
//$formSetup->newItem('DEBWEB_MYPARAM8')->setAsProduct();
...
//$formSetup->newItem('DEBWEB_MYPARAM9')->setAsSelect($TField);
...
$myTmpObjects['debweb'] = array('label'=>'DebWeb', 'includerefgeneration'=>1, 'includedocgeneration'=>0, 'class'=>'DebWeb');
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/debweb/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=debweb-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DEBWEB_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DEBWEB_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-debweb page-admin');
...
$head = debwebAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "debweb@debweb");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DebWebSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DEBWEB_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DEBWEB_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
