...
/**
 *  \file       debweb_agenda.php
 *  \ingroup    debweb
 *  \brief      Tab of events on DebWeb
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->debweb->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	$title = $langs->trans("DebWeb")." - ".$langs->trans('Agenda');
	//$title = $object->ref." - ".$langs->trans("Agenda");
	$help_url = 'EN:Module_Agenda_En|DE:Modul_Terminplanung';

	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-debweb page-card_agenda');
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("DebWeb"), -1, $object->picto);
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		//$cachekey = 'count_events_debweb_'.$object->id;
