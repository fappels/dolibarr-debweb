...
/**
 *    \file       debweb_card.php
 *    \ingroup    debweb
 *    \brief      Page to create/edit/view debweb
 */
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('debweb', 'debweb', 'read');
	$permissiontoadd = $user->hasRight('debweb', 'debweb', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('debweb', 'debweb', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('debweb', 'debweb', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('debweb', 'debweb', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->debweb->multidir_output[isset($object->entity) ? $object->entity : 1].'/debweb';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/debweb/debweb_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/debweb/debweb_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'DEBWEB_MYOBJECT_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'DEBWEB_MYOBJECT_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_MYOBJECT_TO';
	$trackid = 'debweb'.$object->id;
...
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("DebWeb"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-debweb page-card');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("DebWeb"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("DebWeb"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteDebWeb'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionDebWeb', $object->ref);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/debweb/debweb_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	// preview of items in report
	if ($object->type_declaration == 'expedition' && $object->exporttype == 'deb') {
	...
	$defaulttpldir = '/custom/debweb/tpl';
	if (!empty($object->lines)) {
		$object->printObjectLines($action, $mysoc, null, 0, 0, $defaulttpldir);
	}
		...
		if (empty($reshook)) {
			...
			if ($object->status == $object::STATUS_DRAFT) {
				if (empty($object->table_element_line) || (is_array($object->lines) && count($object->lines) > 0)) {
					print dolGetButtonAction('', $langs->trans('Validate'), 'default', $_SERVER['PHP_SELF'].'?id='.$object->id.'&action=confirm_validate&confirm=yes&token='.newToken().'&amount='.$object->amount, '', $permissiontoadd);
				} else {
					$langs->load("errors");
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->debweb->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('debweb:DebWeb', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('debweb'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/debweb/debweb_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->debweb->dir_output;
	$trackid = 'debweb'.$object->id;
