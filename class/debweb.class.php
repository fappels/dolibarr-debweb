...
/**
 * \file        class/debweb.class.php
 * \ingroup     debweb
 * \brief       This file is a CRUD class file for DebWeb (Create/Read/Update/Delete)
 */
 ...
 * Class for DebWeb
 */
class DebWeb extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'debweb';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'debweb';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'debweb_debweb';

	/**
	 * @var string 	If permission must be checkec with hasRight('debweb', 'read') and not hasright('mymodyle', 'debweb', 'read'), you can uncomment this line
	 */
	...
	public $fields=array(
		...
		"exporttype" => array("type"=>"varchar(64)", "label"=>"ExportType", "enabled"=>"1", 'position'=>32, 'notnull'=>1, "arrayofkeyval"=>array("deb" => "DEBexport", "des" => "DESexport"),"visible"=>"1",),
		"type_declaration" => array("type"=>"varchar(64)", "label"=>"TypeDeclaration", "enabled"=>"1", 'position'=>34, 'notnull'=>1, "visible"=>"1", "arrayofkeyval"=>array("introduction" => "Introduction", "expedition" => "Expedition"), "default"=>"expedition",),
		"period_month" => array("type"=>"varchar(64)", "label"=>"PeriodMonth", "enabled"=>"1", 'position'=>36, 'notnull'=>0, "visible"=>"1",),
		"period_year" => array("type"=>"varchar(64)", "label"=>"PeriodYear", "enabled"=>"1", 'position'=>38, 'notnull'=>0, "visible"=>"1",),
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('debweb', 'debweb', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('debweb', 'debweb', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('debweb', 'debweb_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'debweb/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'debweb/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'debweb/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'debweb/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->debweb->dir_output.'/debweb/'.$oldref;
				$dirdest = $conf->debweb->dir_output.'/debweb/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->debweb->dir_output.'/debweb/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('debweb','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('debweb','debweb_advance','validate'))))
		 {
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DEBWEB_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'DEBWEB_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'DEBWEB_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDebWeb")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DebWeb").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/debweb/debweb_card.php', 1).'?id='.$this->id;
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('DEBWEB_MYOBJECT_ADDON')) {
			...
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/debweb/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/debweb/doc/";
