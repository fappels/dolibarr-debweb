...
/**
 *  \file       debweb_document.php
 *  \ingroup    debweb
 *  \brief      Tab for documents linked to DebWeb
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->debweb->multidir_output[$object->entity ? $object->entity : $conf->entity]."/debweb/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('debweb', 'debweb', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("DebWeb")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-debweb page-card_document');

// Show tabs
$head = debwebPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("DebWeb"), -1, $object->picto);
...
$modulepart = 'debweb';
$param = '&id='.$object->id;
//$relativepathwithnofile='debweb/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'debweb/'.dol_sanitizeFileName($object->ref).'/';
